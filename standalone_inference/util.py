# Auto-generated standalone utilities for YOLO-Stutter inference
import argparse
import json
import math
import os
...
attentions = sys.modules[__name__]

# Monotonic alignment (Python fallback)
...
def maximum_path(neg_cent, mask):
    ...
    for i in range(path.shape[0]):
        _maximum_path_each(path[i], neg_cent[i], t_t_max[i], t_s_max[i])
    return torch.from_numpy(path).to(device=device, dtype=dtype)

class _MonotonicAlign:
    @staticmethod
    def maximum_path(neg_cent, mask):
        return maximum_path(neg_cent, mask)

monotonic_align = _MonotonicAlign()
...
class SynthesizerTrn(nn.Module):
    ...
    def __init__(self,
        ...
        # print("spec_channels:{}".format(spec_channels))
        # print("inter_channels:{}".format(inter_channels))
...
def english_cleaners(text):
  ...
  phonemes = phonemize(text, language='en-us', backend='espeak', strip=True, njobs=1)
...
def english_cleaners2(text):
  ...
  text = lowercase(text)
  text = expand_abbreviations(text)
  phonemes = phonemize(text, language='en-us', backend='espeak', strip=True, preserve_punctuation=True, with_stress=True, njobs=1)
  phonemes = collapse_whitespace(phonemes)
  return phonemes
...
def load_checkpoint(checkpoint_path, model, optimizer=None):
    ...
    saved_state_dict = checkpoint_dict['model'] if 'model' in checkpoint_dict else checkpoint_dict
...
def compute_spec(audio, sampling_rate=TARGET_SAMPLE_RATE):
    ...
    normalized_waveform = audio / peak if peak > 0 else audio
...
class DecoderBlock(nn.Module):
    def __init__(self, text_channels, kernel_size, kernel_stride, downsample=False, residual=False):
        super().__init__()
    ...
    def forward(self, x):
        out = self.conv1(torch.transpose(x, -1, -2))
        out = self.bnorm(out)
        out = self.gelu(out)
        out = self.conv2(out)
...
class Conv1DTransformerDecoder(nn.Module):
    def __init__(
        ...
        self.proj = nn.Linear(text_channels, 3 + num_classes)
...
def register_decoder_aliases():
    import sys
    import types

    utils_mod = types.ModuleType("utils")
    model_utils_mod = types.ModuleType("utils.model_utils")
    conv_mod = types.ModuleType("utils.model_utils.conv1d_transformer")

    conv_mod.GELU = GELU
    conv_mod.PositionalEncoding = PositionalEncoding
    conv_mod.DecoderBlock = DecoderBlock
    conv_mod.Conv1DTransformerDecoder = Conv1DTransformerDecoder

    model_utils_mod.conv1d_transformer = conv_mod
    utils_mod.model_utils = model_utils_mod

    sys.modules.setdefault("utils", utils_mod)
    sys.modules.setdefault("utils.model_utils", model_utils_mod)
    sys.modules.setdefault("utils.model_utils.conv1d_transformer", conv_mod)
